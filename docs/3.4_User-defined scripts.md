...
The storage name must be declared and configured in the main config in the `storage_connections` block
in nxs-backup.yml:
...
# deferred_copying is not used in "external" jobs
# The option allows you to postpone copying until all backup parts of the job are completed before its delivery
