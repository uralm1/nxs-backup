...
the `/var/www` directory with an exclusion of the log files.
```yaml
...
The used storage name must be declared and configured in the main config in the `storage_connections` block in nxs-backup.yml:
